        ...
        <div class="nsc-commonBlockBody">
          <div class="selectType">
            ...
            <select name="topicTag" v-model="publishData.topicTag">
              ...
              <option
                v-for="(topicTag, tagIndex) in topicTags"
                :key="tagIndex"
                :value="topicTag.value"
              >
                {{ topicTag.name }}
              </option>
            </select>
          ...
          <div class="topicTitle">
            <input
              type="text"
              v-model.trim="publishData.topicTitle"
              placeholder="标题字数10字以上"
            />
          </div>
          <!-- 选项卡切换按钮 -->
          <div
            @click="switchCard('edit')"
            :class="['switchCard', { switchCardActive: !isPreview }]"
          >
            编辑内容
          </div>
          <div
            @click="switchCard('preview')"
            :class="['switchCard', { switchCardActive: isPreview }]"
          >
            预览内容
          </div>
          <div class="topicContent">
            <textarea
              v-show="!isPreview"
              v-model.trim="publishData.topicContent"
              placeholder="(支持markdown语法) 测试帖请选择“客户端测试”板块！"
            ></textarea>
            <vue-markdown
              v-show="isPreview"
              :source="
                publishData.topicContent.replace('](www', '](https://www')
              "
              class="markdownDisplay md-content"
            ></vue-markdown>
          </div>
          <!-- newPublish按钮 -->
          <button
            v-if="!isModify"
            :disabled="subLoading"
            @click="publishTopic('newPublish')"
            class="big-submit-btn bgc-wineRed block-center"
          >
            <span v-if="!subLoading">发布</span>
            <span v-if="subLoading"
              ><span class="fa fa-spinner fa-spin"></span> 发布中…</span
            >
          </button>
          <!-- modifyPublish按钮 -->
          <button
            v-if="isModify"
            :disabled="subLoading"
            @click="publishTopic('modify')"
            class="big-submit-btn bgc-wineRed block-center"
          >
            <span v-if="!subLoading">提交修改</span>
            <span v-if="subLoading"
              ><span class="fa fa-spinner fa-spin"></span> 提交中…</span
            >
          </button>
        </div>
      </div>
...
export default {
        ...
        if (this.publishData.topicTitle) {
          ...
          if (re.test(this.publishData.topicTitle)) {
            ...
            if (this.publishData.topicContent) {
              ...
              this.publishData.topicContent = this.publishData.topicContent.replace(
                "](www",
                "](https://www"
              );
      ...
      this.$apiRequest.createTopic(
        ...
        res => {
          ...
          if (confirm("发布成功，需要查看刚发布的文章吗？")) {
            this.$router.push({
              path: "/itemCommunity/itemjsTopics/" + res.data.topic_id
            });
          } else {
            // 重新加载该子组件，清空填入内容
            this.$parent.reload();
  ...
  created() {
    ...
    if (this.$route.path.split("/").pop() === "createTopic") {
      this.$commonUtil.exchangePageTitle("在宫墙发布话题");
    }

    ...
    <div v-show="!loading" class="nsc-commonBody">
      <div class="nsc-commonBlockWrp">
        ...
        <div v-if="msgData.hasnotReadMsgs.length" class="nsc-commonBlockBody">
          ...
          <button @click="markHasReadMsg" class="mid-submit-btn-ato bgc-lGray">
            标记选中为已读
          </button>
          <div
            v-for="(notReadMsg, notReadMsgIndex) in msgData.hasnotReadMsgs"
            :key="notReadMsgIndex"
            class="msgItem"
          >
            <!-- 展示未读消息 -->
            <input
              ref="checkMsg"
              @click="markMsg(notReadMsgIndex)"
              type="checkbox"
              v-model="checkedMsg[notReadMsgIndex]"
            />
            <router-link
              :to="{
                name: 'UserRoute',
                params: { name: notReadMsg.author.loginname }
              }"
            >
            ...
            <router-link
              @click.native="markOneMsgToRead(notReadMsgIndex)"
              :to="{
                name: 'ArticleRoute',
                params: { id: notReadMsg.topic.id }
              }"
              >{{ notReadMsg.topic.title }}</router-link
            >
      ...
      <div class="nsc-commonBlockWrp">
        ...
        <div class="nsc-commonBlockBody">
          <div
            v-if="msgData.hasReadMsgs.length"
            v-for="(readMsg, readMsgIndex) in msgData.hasReadMsgs"
            :key="readMsgIndex"
            class="msgItem"
          >
            <!-- 展示已读消息 -->
            <router-link
              :to="{
                name: 'UserRoute',
                params: { name: readMsg.author.loginname }
              }"
            >
            ...
            <router-link
              :to="{ name: 'ArticleRoute', params: { id: readMsg.topic.id } }"
              >{{ readMsg.topic.title }}</router-link
            >
...
export default {
  ...
  methods: {
    ...
    // 发送单条消息标记请求
    markOneMsgToRead(msgIndex) {
      const msgId = this.msgData.hasnotReadMsgs[msgIndex]._id

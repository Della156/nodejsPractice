<template>
  <div ref="article" class="nsc-commonWrp">
    ...
    <div v-show="displayArticleContent" class="nsc-commonBody">
      ...
      <div class="nsc-commonBlockWrp">
        ...
        <div class="articleHeadBody">
          ...
          <router-link
            :to="{
              name: 'UserRoute',
              params: { name: article.author.loginname }
            }"
          >
            <img
              class="articleAuthorAvatar"
          ...
          <div class="articleBaseInfo font-green">
            ...
            <span>{{ newLoginname }}</span
            ><br />
            <i class="fa fa-clock-o fa-fw"></i>
            <span>{{ article.create_at.slice(0, 10) }}</span
            ><br />
          ...
          <div class="articleControlPanel">
            <!-- 收藏按钮 -->
            <button
              :disabled="subLoading.collectLoading"
              @click="collectController()"
              :class="[
                'mid-submit-btn',
                'bgc-green',
                'float-r',
                { 'bgc-lGray': collectBtnActive }
              ]"
              :title="collect"
            >
              <span v-if="!subLoading.collectLoading">{{ collect }}</span>
              <span
                v-if="subLoading.collectLoading"
                class="fa fa-spinner fa-spin"
              ></span>
            </button>
            <!-- 编辑按钮 -->
            <button
              @click="editController('edit')"
              v-if="article.author.loginname === $store.state.loginUsername"
              class="mid-submit-btn float-r bgc-lGray"
              title="点击编辑"
            >
      ...
      <!-- 文章详情页文章内容板块 -->
      <vue-markdown
        class="nsc-commonBlockWrp md-content"
      ...
      <div ref="comments" class="nsc-commonBlockWrp">
        ...
        <div class="publishCommnetWrp">
          <div v-if="!loginStatus" class="unLogin">
            ...
            <div
              @click="$store.commit('openLoginCard', true)"
              class="mid-submit-btn-ato bgc-green float-r"
              title="点此登录"
            >
              登录
            </div>
          ...
          <div v-if="loginStatus">
            ...
            >
            <button
              :disabled="subLoading.commentLoading"
              @click="publishComment"
              class="mid-submit-btn bgc-wineRed float-r"
              title="点此发送"
            >
        ...
        <div class="nsc-commonBlockBody">
          ...
          <!-- 有评论时的内容 -->
          <div
            v-if="article.replies.length"
            v-for="(comment, index) in article.replies"
            :key="index"
            class="commentCell"
          >
            <div>
              <router-link
                :to="{
                  name: 'UserRoute',
                  params: { name: comment.author.loginname }
                }"
                class="flex-box-row float-l"
              >
                <img
                ...
                <span
                  v-if="article.author.loginname === comment.author.loginname"
                  class="mark-tag bgc-green"
                  >作者</span
                >
              </router-link>
            </div>
            <vue-markdown
              ...
              :source="
                comment.content.replace(
                  '(/user/',
                  '(' + currentUrlPrefix + 'itemCommunity/user/'
                )
              "
            ...
            <div>
              ...
              <button
                ...
                :class="[
                  ...
                  'like',
                  {
                    likeActive: comment.is_uped,
                    'fa-thumbs-o-up': !comment.is_uped,
                    'fa-thumbs-up': comment.is_uped
                  }
                ]"
              >
                {{ comment.ups.length }}
    ...
    <!-- 文章编辑组件 -->
    <edit-topic
      :topicTag="article.tab"
      :topicTitle="article.title"
      :topicContent="article.content"
      v-if="eidtWindow"
    >
      <div slot="modifyBlockTitle">
        <h2>话题编辑</h2>
        <span
          @click="editController('cancelEdit')"
          class="mid-submit-btn bgc-white cancelEdit"
          >取消编辑</span
        >
      </div>
    </edit-topic>
    ...
    <div v-show="!eidtWindow" class="anchorBtnWrp">
      <span
        @click="anchorPosition('top')"
        class="fa fa-arrow-circle-up circle-btn bgc-wineRed"
        title="回到顶部"
      ></span>
      <span
        @click="anchorPosition('comments')"
        class="fa fa-commenting circle-btn bgc-wineRed"
        title="查看评论"
      ></span>
    </div>
    <!-- 页面加载loading -->
...
export default {
  ...
  methods: {
    ...
    likeController: function(replyIndex) {
      ...
      if (!this.loginStatus) {
        ...
        this.article.replies[replyIndex].author.loginname
      ) {
        alert("呵呵，不能给自己点赞");
      } else {
  ...
  created: function() {
    ...
    this.$apiRequest.getTopicDetail(
      ...
      err => {
        this.$commonUtil.netErrorTips(err);
        this.$router.push({ path: "/itemCommunity/itemjsTopics" });
      }
    );
    ...
    if (RegExp(/localhost:/).test(tempUrlPart)) {
      ...
      this.currentUrlPrefix =
        "https://reviving-pain.gitee.io/cnodejs-community/#/";
    }
  }
};
</script>

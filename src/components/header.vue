  ...
  <div class="header" ref="navHead">
    ...
    <span
      ...
      :class="[
        ...
        'fa-th-list',
        'fa-th-list-response',
        { 'navSide-appear': $store.state.showNavside }
      ]"
    ></span>
    <!-- 头像相关内容 -->
    <div @click="showMenu" class="userAvatar userAvatar-response">
      <span
        v-if="!loginStatus"
        class="defalutAvatar fa fa-user-circle"
        title="点击登录"
      ></span>
      <img
        v-if="loginStatus"
        :src="$store.state.loginAvatarUrl"
        :title="$store.state.loginUsername"
      />
    </div>
    <!-- 头像展开菜单 -->
    <div v-show="showAvatarMenu" class="avatarMenu avatarMenu-response">
      ...
      <div class="menuBody">
        <ul @click="avataMenuMethod">
          ...
          <li id="userCenter" v-if="loginStatus" class="fa fa-user-circle-o">
            个人中心
          </li>
          <li id="logout" v-if="loginStatus" class="fa fa-sign-out">
            退出登录
          </li>
        </ul>
      </div>
  ...
  methods: {
    ...
    avataMenuMethod(e) {
      ...
      switch (e.target.id) {
        ...
        case "userCenter":
          console.log("前往个人中心");
          if (this.$route.path !== "/itemCommunity/profile") {
            this.$router.push({ path: "/itemCommunity/profile" });
          }
          break;
        case "logout":
    ...
    loadMsgPage() {
      if (this.$store.state.loginStatus) {
        if (this.$route.path !== "/itemCommunity/messages") {
          this.$router.push({ path: "/itemCommunity/messages" });
        }
      } else {
        alert("您尚未登陆，请先登录");
        this.$store.commit("openLoginCard", true);

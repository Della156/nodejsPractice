<template>
  <div
    :class="[
      'nav-side-wrp',
      'nav-side-response',
      { 'nav-side-control': this.$store.state.showNavside }
    ]"
  >
    <!-- 一级菜单选项 -->
    <transition-group class="tabs" tag="ul">
      <li
        @click="selectTabItem(index, tab)"
        class="tabs-item"
      ...
      >
        ...
        <span
          ...
          :class="[
            'fa',
            'fa-chevron-right',
            { 'chevron-right-rotate': index + 1 === selectedTabNum }
          ]"
        ></span>
        <!-- 二级菜单选项 -->
        <ul
          v-if="tab.subTabs"
          :class="['subTabs', { subTabsOpen: index + 1 === selectedTabNum }]"
        >
          <li
            @click.stop="selectSubtabItem(tab.path, subTab.path)"
            :class="[
              'subTabs-item',
              { 'subTab-active': tab.path + subTab.path === $route.path }
            ]"
...
export default {
  data: function() {
    return {
      ...
      isClick: false,
      shouldLoginComponents: ["createTopic", "profile"]
    };
  },
  methods: {
    // 选中一级菜单时触发的事件
    selectTabItem(tabIndex, tab) {
    ...
    selectSubtabItem(tabPath, subTabPath) {
      this.isClick = true;
      let shouldLogin =
        this.shouldLoginComponents.indexOf(subTabPath.slice(1)) + 1;
      if (this.loginStatus) {
